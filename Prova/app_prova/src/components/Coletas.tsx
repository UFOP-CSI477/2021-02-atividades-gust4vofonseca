...
export default function Coletas () {
    ...
    function loadItemEntidades() {
        api.get('/entidades/list-all').then(response => {
            setEntidades(response.data);
        })
        api.get('/items/list-all').then(response => {
            setItems(response.data);
        })
        api.get('/coletas/list-all').then(response => {
    ...
    return (
        <div>
            <h1>Coletas</h1>
            {!cadastrar &&  (<button onClick={() => setCadastrar(!cadastrar)} className="buttonCreate">Cadastrar</button>)}
            { cadastrar &&
            ...
            <h1>Cadastradas</h1>
            {
                coletas.map(coleta => (
                    <div className="containerList" id={coleta.id}>
                        ...
                        <button className="buttonDeleteInput" onClick={() => deleteColeta(coleta.id)}>Deletar</button>
                        { update !== coleta.id && <button className="buttonUpdateInput" onClick={() => setUpdate(coleta.id)}>Editar</button>}
                        {
                            update === coleta.id && 
                            <div className="container login">
